...
\title{\textbf{\texttt{ropfec}: a tested, reproducible implementation of
...
concept without an open, tested implementation. We present \texttt{ropfec}, a small,
...
\textbf{(ii)}~an honest account of where its structural prior actually helps. \texttt{ropfec}
...
estimation (Section 3). Self-skepticism is shipped as a re-runnable feature, so
...
(Figure~2), and the FEC solver, tracking a target flux, converges to
...
\begin{figure}[t]
\includegraphics[width=12cm]{figures/order_uq_negative.png}
\caption{\textbf{Figure 4.} Built-in falsification (the package's central result). Total reaction-order
...
archived with a DOI on Zenodo upon posting (Table~1).
