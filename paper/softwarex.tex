...
the binding-derived ROP does \emph{not materially} improve data-driven estimation of reaction orders.
...
another~\citep{xiao2023fec,xiao2022thesis,xiao2021structure,xiao2018rpa}: in a power-law description the reaction-order
...
a count-matched random-facet null and finds it confers at most a marginal advantage for
data-driven order estimation (\S\ref{sec:examples}). Self-skepticism is shipped as a re-runnable feature, so
...
MM-derived exponent policies ($1.01$ vs $1.28$ and $1.09$; a single deterministic evaluation,
all three respecting the polytope), and a scenario-robust variant bounds the worst case at near-nominal cost --- internal
...
(60 seeds) --- a difference of only $\sim$1\% that lies well within the cross-seed scatter
($\pm0.58$). The binding-derived structure therefore adds little to identification beyond a
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{order_uq_negative.pdf}
...
(2.31 vs 2.34), well within the cross-seed scatter. Shipped as a re-runnable test.}
\label{fig:neg}
\end{figure}
