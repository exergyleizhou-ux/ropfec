...
\title{\textbf{\code{ropfec}: an open, tested implementation of
...
\begin{abstract}
\noindent
We present \code{ropfec}, a small, well-tested Python package that makes
...
C3 Permanent link to reproducible capsule & \href{https://doi.org/10.5281/zenodo.20805373}{10.5281/zenodo.20805373} (Zenodo, \texttt{v1.0.1}) \\
C4 Legal code license & MIT \\
C5 Code versioning system used & git \\
C6 Software languages, tools, services & Python~3.9+, NumPy, SciPy, CasADi/IPOPT, Matplotlib, pytest \\
C7 Compilation/runtime requirements \& dependencies & Python~$\ge$3.9; NumPy, SciPy, Matplotlib; CasADi+IPOPT optional (SciPy projection fallback otherwise); plus the bundled in-repo \code{bos\_platform} reference package (installed by \code{pip install -e .}), used by the benchmark/integration tests and the Example-1 costs \\
...
\textbf{(ii)}~an account of where its structural prior actually helps. \code{ropfec}
...
data-driven order estimation (\S\ref{sec:examples}). This null comparison ships as a runnable test, so
...
\code{examples/} benchmark, not by this script). Installation and reproduction are environment-agnostic:
\begin{verbatim}
...
(Figure~\ref{fig:toy}), and the FEC solver, tracking a target
...
The order-uncertainty module exposes importable primitives (\code{worst\_case\_range},
\code{bounding\_box}, \code{data\_halfspaces}, \code{estimate\_orders\_loglinear}), and the two
...
\url{https://github.com/exergyleizhou-ux/ropfec} (Table~\ref{tab:meta}); a versioned snapshot
of the \texttt{v1.0.1} release is archived on Zenodo (\href{https://doi.org/10.5281/zenodo.20805373}{10.5281/zenodo.20805373}; concept DOI 10.5281/zenodo.20805372).
